        ...
        <div class="questions">
            @if(empty(old('question')))
                <div>
                    ...
                    <x-text-input id="question0" class="block mt-1 w-full" type="text" name="question[][body]" :value="old('question.0')" required autofocus autocomplete="name" />
            ...
            @else
                @foreach(old('question') as $key => $value)
                    ...
                    <x-text-input id="question{{ $key }}" class="block mt-1 w-full" type="text" name="question[][body]" :value="old('question.{{ key }}')" required autofocus autocomplete="name" />
                    <x-input-error :messages="$errors->get('question.body')" class="mt-2" />
                @endforeach
            @endif
        </div>
